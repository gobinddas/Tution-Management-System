...
const AddFee = ({ onDone }) => {
  ...
  useEffect(() => {
    setStudent(students.find(s => s.id === selectedStudentId));
  ...
  return (
    <div className="flex flex-col md:flex-row gap-10 bg-white/90 rounded-3xl shadow-2xl p-8 md:p-12 border border-blue-100">
      ...
      <form className="flex-1 space-y-6" onSubmit={handleSubmit}>
        <div>
          <label className="block font-semibold mb-1 text-blue-700">Select Student</label>
          <select
          ...
          >
            ...
            {students.map(s => (
              <option key={s.id} value={s.id}>
                {`${s.firstName || ''} ${s.middleName || ''} ${s.lastName || ''}`.replace(/\s+/g, ' ').trim()} ({s.batch || 'No batch'})
              </option>
            ))}
          </select>
        </div>
