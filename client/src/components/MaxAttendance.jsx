...
const MaxAttendance = ({ batchId, students, attendanceRecords }) => {
  ...
  if (studentPercentages.length === 0) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center text-green-700 font-semibold shadow">
        No student has more than 70% attendance.
      </div>
    );
  }
  ...
  return (
    <div className="bg-green-100 border border-green-400 rounded-xl p-4 shadow">
      <h3 className="text-lg font-bold mb-3 text-green-800">Max Attendance (&gt; 70%)</h3>
        ...
        <table className="min-w-full bg-white rounded-xl overflow-hidden">
          ...
          <tbody>
            {studentPercentages.map((s) => (
              <tr key={s.id} className="even:bg-green-50">
